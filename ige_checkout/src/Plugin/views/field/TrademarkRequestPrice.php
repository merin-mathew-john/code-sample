 ...
 *
 * @ViewsField("ige_checkout_trademark_price")
 */
class TrademarkRequestPrice extends FieldPluginBase {
  ...
  public function render(ResultRow $values) {
    $trademark_request = $values->_entity;
    if ($trademark_request->field_admin_added->value) {
      $data = $this->checkoutHelper->getAdditionalTrademarkRequestData([$trademark_request]);
    }
    else {
      $data = $this->checkoutHelper->getTrademarkRequestData([$trademark_request]);
    }

    return $data[$trademark_request->id()]['price'];
